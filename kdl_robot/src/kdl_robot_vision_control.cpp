...
int main(int argc, char **argv)
{
    ...
    ros::Publisher s_pub = n.advertise<geometry_msgs::Vector3>("/kdl_robot_vision/s",2);
    ...
    while (true)
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            //Control Law
            dqd.data=2*LJ_pinv*Eigen::Vector3d(0,0,1) + 2*N*(qdi - toEigen(jnt_pos));
            ...
            // std::cout << "s.norm: "<<std::endl << s.norm()<<std::endl;
